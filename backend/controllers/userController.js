...
export const getUserProfile = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error while getting profile" });
  }
};
...
export const getSharedCalendars = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Server error while getting shared calendars" });
  }
};
...
export const shareCalendar = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Calendar shared successfully", user });
  } catch (error) {
    console.error("Share calendar error:", error);
    return res
      .status(500)
      .json({ message: "Server error while sharing calendar" });
  }
};
